...
app.get('/api/projects', async (req, res) => {
  try {
    ...
    // If no status filter specified, show all projects (not just published)
    if (year) {
...
app.post('/api/projects', async (req, res) => {
  try {
    // Identify the user: either from a token or from lead/email in the body
    let userId;
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const articleResults = await articlesCollection
      .find({
        $or: [{ title: regex }, { description: regex }, { content: regex }, { category: regex }],
      })
      .sort({ createdAt: -1 })
      .limit(30)
      .toArray();
