...
// MongoDB driver v5/v6 returns the updated document directly from
// findOneAndUpdate (or null), not wrapped in `{ value }` like legacy v3/v4
// did. This normalizes both shapes so the rest of the code doesn't care
// which driver version is installed. Without this, `.value` is silently
// `undefined` on the current driver even when the update itself succeeded,
// which was causing routes to report failure (or skip follow-up logic
// entirely) despite the database write going through fine.
function getUpdatedDoc(result) {
...
app.put("/api/users/:id", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const user = getUpdatedDoc(result);
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      const project = getUpdatedDoc(result);
      if (!project) return res.status(404).json({ error: "Project not found" });
      cacheInvalidate("projects:fast:");
      cacheInvalidate("projects:list:");
      res.json({ project });
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedProject = getUpdatedDoc(result);
      if (!updatedProject)
...
app.put("/api/labs/:id", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const lab = getUpdatedDoc(result);
    if (!lab) return res.status(404).json({ error: "Lab not found" });
    res.json({ lab });
  } catch (err) {
    console.error(err);
...
app.put("/api/articles/:id", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const article = getUpdatedDoc(result);
    if (!article) return res.status(404).json({ error: "Article not found" });
    res.json({ article });
  } catch (err) {
    console.error(err);
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      const submission = getUpdatedDoc(result);
      if (!submission)
        return res.status(404).json({ error: "Submission not found" });
...
app.post("/api/form-submissions/lead-confirm", async (req, res) => {
  try {
    ...
    const updatedResult = await formSubmissionsCollection.findOneAndUpdate(
      { _id: submission._id },
      {
        $set: {
    ...
    const updated = getUpdatedDoc(updatedResult);
    if (!updated)
      return res.status(500).json({ error: "Unable to confirm assignment" });

    const studentUserEmail = updated.email.toLowerCase().trim();
    let studentUser = await usersCollection.findOne({
      email: studentUserEmail,
    });
    if (!studentUser) {
      const result = await usersCollection.insertOne({
        name: updated.name || "Research Student",
        email: studentUserEmail,
    ...
    if (updated.projectId && studentUser) {
    ...
    let confirmedProjectSlug = updated.projectSlug || "";
    let confirmedProjectPageLink = `${FRONTEND_URL}/projects`;
    if (!confirmedProjectSlug && updated.projectId) {
      const project = await projectsCollection.findOne(
        { _id: new ObjectId(updated.projectId) },
        { projection: { slug: 1 } },
      );
    ...
    if (confirmedProjectSlug) {
      ...
      confirmedProjectPageLink = `${FRONTEND_URL}/projects/${updated.projectId}`;
    }
    const studentSubject = "Your research project assignment is confirmed";
    const studentText = `Hello ${updated.name},\n\nGood news: the project lead has confirmed your assignment to ${updated.projectTitle || "the requested research project"}. You are now part of that research effort.\n\nYou can view the project here: ${confirmedProjectPageLink}\n\nBest of luck.`;
    const studentHtml = `<p>Hello ${updated.name},</p><p>Good news: the project lead has confirmed your assignment to ${updated.projectTitle ? `<strong>${updated.projectTitle}</strong>` : "the requested research project"}.</p><p>You can view the project here: <a href="${confirmedProjectPageLink}">${confirmedProjectPageLink}</a></p><p>You are now part of that research effort.</p><p>Best of luck.</p>`;
    await sendMail({
      to: updated.email,
      subject: studentSubject,
      text: studentText,
