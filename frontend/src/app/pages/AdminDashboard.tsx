...
export function AdminDashboard() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://hi-there--alisorab423.replit.app'}/api/form-submissions?${params}`,
        {
          headers: {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://hi-there--alisorab423.replit.app'}/api/form-submissions-stats`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const handleStatusChange = async (submissionId: string, newStatus: string) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://hi-there--alisorab423.replit.app'}/api/form-submissions/${submissionId}`,
        {
          method: 'PUT',
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://hi-there--alisorab423.replit.app'}/api/form-submissions/${submissionToDelete}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
